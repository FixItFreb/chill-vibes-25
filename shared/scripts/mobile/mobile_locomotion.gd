...
func _physics_process(delta: float) -> void:
	...
	if !face_move_direction:
		direction = (mobile_body.transform.basis * Vector3(input_dir.x, 0, input_dir.y)).normalized()
	else:
		direction = (get_viewport().get_camera_3d().transform.basis * Vector3(input_dir.x, 0, input_dir.y)).normalized()
